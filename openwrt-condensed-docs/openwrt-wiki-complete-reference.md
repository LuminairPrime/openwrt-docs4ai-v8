...
> **Generated:** 2026-03-05 18:33 UTC
...
4.  Image Configuration: Use this section to set up the defaults for the LAN ports on your router. If you don't have a serial cable connected to the router, you'll need to do this so that you can access the router once your newly-flashed code boots up. I recommend entering values for DNS server (on your ISP's network), LAN network mask (for the addresses to be doled out via DHCP), and LAN IP address (private IP address on your router's side).
    ...
    - NOTE: It is not possible to set this feature in the latest [attitude adjustment](https://dev.openwrt.org/changeset/31258/) release. See this [mailing list post](http://www.mail-archive.com/openwrt-devel@lists.openwrt.org/msg15992.html) for more information.
...
:!: Be aware there are default compiler options defined in include/[target.mk](/openwrt-buildroot-docs/openwrt-buildroot-include-mk.md) for example ("-Os -pipe")
...
`make package/feeds/<feed_name>/<package_name>`
    ...
    Commands:
            ...
            Options:
                ...
                -f :            List feeds in opkg [feeds.conf](/openwrt-wiki-docs/openwrt-guide-developer-creating-a-meson-based-package.md) compatible format (when using -s).
            ...
            update -a|<feedname(s)>: Update packages and lists of feeds in [feeds.conf](/openwrt-wiki-docs/openwrt-guide-developer-creating-a-meson-based-package.md) .
            Options:
                -a :           Update all feeds listed within [feeds.conf](/openwrt-wiki-docs/openwrt-guide-developer-creating-a-meson-based-package.md). Otherwise the specified feeds will be updated.
...
- you could access the hardware *without any booting, without any software present*, over the [JTAG Port](/docs/techref/hardware/port.jtag) with JTAG Software, like HairyDairyMaid
...
NOTE: Some of the information provided in this Wiki might be redundant from <https://github.com/openwrt/luci/wiki/ModulesHowTo>
...
- '' json_dump ''
- '' json_init ''
- '' [json_get_var](/openwrt-wiki-docs/openwrt-guide-developer-jshn.md) ''
- '' json_get_vars ''
...
Add/update the "mypackages" custom feed and selinux-policy-myfork:
    ...
    [kcinimod@brutus ~]$ echo "src-link custom ${HOME}/mypackages" >> openwrt/[feeds.conf](/openwrt-wiki-docs/openwrt-guide-developer-creating-a-meson-based-package.md).default
...
- [coreboot](https://en.wikipedia.org/wiki/coreboot) \<color green\>GPLv2\</color\> successor of the BIOS, alternative to UEFI based on the Linux kernel;  
...
The available Kernel version are listed in include/kernel-[version.mk](/openwrt-buildroot-docs/openwrt-buildroot-include-mk.md):
...
- symlink approach
- [mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md) [r3667 add mini_fo patches to mount_root and firstboot](https://dev.openwrt.org/changeset/3667) and [r3669 mini_fo works](https://dev.openwrt.org/changeset/3669) and [r3928 enabled mini_fo by default](https://dev.openwrt.org/changeset/3928)
...
There is a generic problem when running SquashFS on NAND: The issue is that SquashFS has no bad block management at all and requires all blocks on order; but for proper NAND bad block management you also need to be able to skip bad blocks and occasionally relocate blocks (see [squashfs and NAND flash](https://www.infradead.org/pipermail/linux-mtd/2006-April/015386.html)). That's why raw SquashFS is a bad idea on NAND (it works if you use a FTL like UBIFS).
...
- [UBIFS](https://en.wikipedia.org/wiki/UBIFS) is a file system for [raw flash](/docs/techref/flash.layout). It is used in OpenWrt NAND targets since :FIXME: around r40364
...
- Some large partitions are used as containers for other compartmentalized data. Note that the amount of bad blocks in a certain partition is a-priory unknown, and thus a raw partition size cannot be taken as the its usable size. For smaller partitions this effect is amplified: although there is a manufacturer-defined limit on the number of bad blocks in a flash, nothing precludes all bad blocks from residing in the same partition. Thus, for guaranteed operation, a system designer should allow *in each and every partition* the maximum number of bad blocks specified for the complete flash. (In practice though, this is almost never done.) Also note that the previous kinds of defect handling do not spread wear produced by erase/write cycles across the whole flash, and thus in general reduce the lifespan of the device. These problems are both solved by UBI. Ideally a single very large UBI partition is created that entirely manages flash defects and wear-leveling for contained volumes, and inside it different UBI volumes are created:
  ...
  - Some UBI volumes are used as read-write filesystems. Only the [UBIFS](/openwrt-wiki-docs/openwrt-techref-filesystems.md) filesystem is used for this. (It would be possible to emulate read-write block devices on top of UBI and use regular filesystems on top of that, but such setups would underperfom compared to [UBIFS](/openwrt-wiki-docs/openwrt-techref-filesystems.md), and it seems that the necessary UBI block emulation driver has not yet been implemented, if ever.)
...
size depends on selected packages</td>
<td><strong><code>rootfs_data</code></strong><br />
mounted: "<code>/overlay</code>", <a href="/docs/techref/filesystems#[UBIFS](/openwrt-wiki-docs/openwrt-techref-filesystems.md)">[UBIFS](/openwrt-wiki-docs/openwrt-techref-filesystems.md)</a><br />
all remaining free space</td>
<td>:::</td>
</tr>
<tr class="odd">
...
Given that this volume is routinely wiped by sysupgrade, storing any remotely valuable files here would be ill-advised. For this router you might choose to limit **`rootfs_data`** to a generous 128 MiB, and create a new 192 MiB [UBIFS](/openwrt-wiki-docs/openwrt-techref-filesystems.md) volume for persistent storage, while still reserving 50+ MiB as free space to accommodate future growth of OpenWrt images. Let's do just that and name the new volume **`extra`**.
...
You just freed 240+ MiB in the UBI partition. Next, you could manually create, format, and mount a new [UBIFS](/openwrt-wiki-docs/openwrt-techref-filesystems.md) volume. But OpenWrt has a tool to automate this, so let's use it.
...
For overlaying a special `mini_fo` filesystem is used, the `README` is available from the sources at <https://dev.openwrt.org/browser/trunk/target/linux/generic/patches-2.6.37/209-[mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md).patch>
...
Inside your platform directory you will need to create a file to tell the buildroot system how to process the results of a compiled kernel. Most of the work is done automatically by [image.mk](/openwrt-buildroot-docs/openwrt-buildroot-include-mk.md) but different platforms and individual devices will need specific work for images to be useful.
...
Example:
    ...
    cat $(LINUX_DIR)/arch/arm/boot/[zImage](/openwrt-wiki-docs/openwrt-techref-image-format.md) >> $(KDIR)/$(call zimage_name,$(1))
...
have a look at your copy of **trunk/include/[image.mk](/openwrt-buildroot-docs/openwrt-buildroot-include-mk.md)**
...
| check_for_mini_fo      | determine if we have [mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md) overlay in kernel. If not run \*no_fo\* hook                                     |
...
| Function                | Description                                                                                             |
|:------------------------|:--------------------------------------------------------------------------------------------------------|
| reset_check_for_mini_fo | Determine if the kernel supports [mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md) overlay                                                        |
| reset_clear_jffs        | if [mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md) is supported, erase all data in overlay and remount (resets back to 'pure' squashfs versions |
| reset_copy_rom          | if [mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md) is not supported, make symlinks and copy critical files from squashfs to jffs                |
...
This results in the following boot log:
      ...
      switching to jffs2
      [mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md): using base directory: /
      [mini_fo](/openwrt-wiki-docs/openwrt-techref-filesystems.md): using storage directory: /jffs
      - init -
...
|-----------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|-------------------|
| [adm6996](https://dev.openwrt.org/browser/trunk/target/linux/generic/files/drivers/net/phy/adm6996.c)     | Infineon/ADMTek 6996M/L/FC                                                           | MDIO / GPIO       |
...
| [rtl8366s](https://dev.openwrt.org/browser/trunk/target/linux/generic/files/drivers/net/phy/rtl8366s.c)   | Realtek RTL8366S                                                                     | MDIO GPIO/SMI     |
| [rtl8366rb](https://dev.openwrt.org/browser/trunk/target/linux/generic/files/drivers/net/phy/rtl8366rb.c) | Realtek RTL8366RB                                                                    | MDIO GPIO/SMI     |
...
See the [Postman collection](https://documenter.getpostman.com/view/5972215/TVzNHeej#154b5d84-4065-4887-a5a8-c1f3b51fe9a6) with some examples of calling UBUS over HTTP
...
Go to the source directory (where the [CMakeLists.txt](/openwrt-wiki-docs/openwrt-guide-developer-creating-a-cmake-package-in-openwrt.md) lives) and optionally configure the build without Lua bindings:
